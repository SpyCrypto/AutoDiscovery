import {
  ComplianceProofSimulator,
  logger
} from "./simulators/compliance-proof-simulator.js";
...
describe("Compliance Proof smart contract", () => {
  it("should attest step-level compliance when completed before deadline", () => {
    ...
    const { ledgerState, attestationHash } = simulator
      .as("p1")
      .attestStepLevelCompliance(CASE_IDENTIFIER, STEP_HASH, FUTURE_DEADLINE);

    logger.info({
      section: "attestStepLevelCompliance",
      totalAttestationsGenerated:
        ledgerState.totalAttestationsGenerated.toString()
    });
    ...
    expect(
      ledgerState.registeredAttestationHashes.member(attestationHash)
    ).toBe(true);
  });
  ...
  it("should store attestation scope level as 1 (step-level) on the ledger", () => {
    ...
    expect(
      ledgerState.attestationScopeLevelByHash.member(attestationHash)
    ).toBe(true);
    expect(
      ledgerState.attestationScopeLevelByHash.lookup(attestationHash)
    ).toEqual(0n);
  });

  it("should verify that an attestation exists on the ledger", () => {
    const simulator = ComplianceProofSimulator.deployContract();
    const { attestationHash } = simulator
      .as("p1")
      .attestStepLevelCompliance(CASE_IDENTIFIER, STEP_HASH, FUTURE_DEADLINE);
    const { ledgerState: _ledgerState, exists } = simulator
      .as("p1")
      .verifyAttestationExists(attestationHash);
  ...
  it("should record the attestation timestamp on the ledger", () => {
    ...
    const storedTimestamp =
      ledgerState.attestationGeneratedTimestampByHash.lookup(attestationHash);
