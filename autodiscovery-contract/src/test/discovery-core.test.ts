import {
  DiscoveryCoreSimulator,
  logger
} from "./simulators/discovery-core-simulator.js";
...
describe("Discovery Core smart contract", () => {
  it("should create a new case and verify public ledger state", () => {
    ...
    const { ledgerState, caseId } = simulator
      .as("p1")
      .createNewCase(CASE_NUMBER, JURISDICTION_CODE);

    logger.info({
      section: "createNewCase",
      caseId: caseId.toString(),
      totalCasesCreated: ledgerState.totalCasesCreated.toString()
    });
    ...
    expect(ledgerState.jurisdictionCodeByCaseIdentifier.member(caseId)).toBe(
      true
    );
    expect(ledgerState.jurisdictionCodeByCaseIdentifier.lookup(caseId)).toEqual(
      JURISDICTION_CODE
    );
  });
  ...
  it("should add a discovery step to a case", () => {
    const simulator = DiscoveryCoreSimulator.deployContract();
    const { caseId } = simulator
      .as("p1")
      .createNewCase(CASE_NUMBER, JURISDICTION_CODE);
    const { ledgerState, stepHash } = simulator
      .as("p1")
      .addDiscoveryStepToCase(caseId, RULE_REFERENCE, FUTURE_DEADLINE);

    logger.info({
      section: "addDiscoveryStepToCase",
      stepHash: stepHash.toString()
    });
  ...
  it("should mark a step as completed and generate attestation", () => {
    ...
    const { caseId } = simulator
      .as("p1")
      .createNewCase(CASE_NUMBER, JURISDICTION_CODE);
    const { stepHash } = simulator
      .as("p1")
      .addDiscoveryStepToCase(caseId, RULE_REFERENCE, FUTURE_DEADLINE);
    const { ledgerState, attestationHash } = simulator
      .as("p1")
      .markDiscoveryStepAsCompleted(caseId, stepHash);

    logger.info({
      section: "markDiscoveryStepAsCompleted",
      stepHash: stepHash.toString(),
    ...
    expect(
      ledgerState.completionAttestationHashes.member(attestationHash)
    ).toBe(true);
  });
  ...
  it("should check case compliance status", () => {
    ...
    const { caseId } = simulator
      .as("p1")
      .createNewCase(CASE_NUMBER, JURISDICTION_CODE);
    const { ledgerState, isCompliant } = simulator
      .as("p1")
      .checkCaseComplianceStatus(caseId);
